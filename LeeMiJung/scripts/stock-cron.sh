...
# 주식시장 브리핑 cron 스크립트
#
# 사용법: stock-cron.sh [morning|lunch|evening]
#
# 스케줄러: launchd LaunchAgent (2026-08-04 cron에서 이전)
#   ~/Library/LaunchAgents/com.ghyu.claude.stock.{morning,lunch,evening}.plist
#   morning 07:30 / lunch 12:30 / evening 17:30 (평일만)
#   수동 실행: launchctl kickstart -k gui/$(id -u)/com.ghyu.claude.stock.lunch
#
# 동작 흐름:
...
LOG_FILE="$PROJECT_DIR/logs/stock-cron.log"
...
# --- 2) 모드별 스킬 실행 ---
case "$MODE" in
  evening) COMMAND="stock-evening 스킬을 실행해줘" ;;
  lunch)   COMMAND="stock-lunch 스킬을 실행해줘" ;;
  *)       COMMAND="stock-morning 스킬을 실행해줘" ;;
esac

echo "=== $(date) | $MODE stock briefing ===" >> "$LOG_FILE"
...
if echo "$OUTPUT" | grep -q "authentication_error\|401\|Invalid authentication"; then
  ...
  "$PROJECT_DIR/scripts/tg-send.sh" -c "${_CHAT_IDS[0]}" \
    "⚠️ 마스터님, 주식 브리핑($MODE) 실행 중 인증 오류가 발생했어요!
